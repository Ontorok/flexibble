...
const ProjectCard = ({
  project: { category, createdBy, description, githubUrl, id, image, liveSiteUrl, title },
}: Props) => {
  ...
  return (
    <div className="flexCenter flex-col rounded-2xl drop-shadow-card">
      <Link href={`/project/${id}`} className="flexCenter group relative w-full h-full">
        <Image src={image} width={414} height={314} alt={title} className="w-full h-full object-cover rounded-2xl" />
